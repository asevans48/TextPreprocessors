...
object HTestDriver{
  ...
  def main(args:Array[String]):Unit={
    val hasher : ParallelFeatureHasher = new ParallelFeatureHasher()
    val parallelHasher :ParallelFeatureHasher = new ParallelFeatureHasher(50000)
    ...
    var tfpar : ParallelTFIDFVectorizer = new ParallelTFIDFVectorizer(parallelHasher)
    tfpar.transform()
    ...
    val cos =  csc.t * csc / (csc.activeValuesIterator.map { x => x * x }.sum)
    println(cos)
  }
}